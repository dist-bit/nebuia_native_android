...
public class ProgressHUD {
    ...
    private IndefiniteAnimatedView indefiniteAnimatedView;
    private ActivityIndicatorView activityIndicatorView;
    private ProgressAnimatedView progressAnimatedView;

    private FrameLayout contentView;
    private ViewGroup backgroundLayer;
    private RadialGradientLayer backgroundGradientLayer;
    ...
    // Customization
    private ProgressHUDStyle defaultStyle;
    private ProgressHUDMaskType defaultMaskType;
    private ProgressHUDAnimationType defaultAnimationType;
    ...
    private void init() {
        ...
        indefiniteAnimatedView = new IndefiniteAnimatedView(activity);
        activityIndicatorView = new ActivityIndicatorView(activity);
        progressAnimatedView = new ProgressAnimatedView(activity);

        contentView = (FrameLayout) layoutInflater.inflate(R.layout.progresshud_layout, null, false);
        backgroundLayer = contentView.findViewById(R.id.backgroundLayerHud);
        backgroundGradientLayer = contentView.findViewById(R.id.backgroundGradientLayerHud);
        hudView = contentView.findViewById(R.id.hud_nebuia_view);
        hudView.setBackground(hudViewBackgroundDrawable);
        imageContainer = contentView.findViewById(R.id.imageContainerHud);
        statusLabel = contentView.findViewById(R.id.statusLabelHud);

        animBackgroundLayerFadeIn = AnimationUtils.loadAnimation(activity, R.anim.progresshud_bg_fade_in);
        animBackgroundLayerFadeOut = AnimationUtils.loadAnimation(activity, R.anim.progresshud_bg_fade_out);

        animHudViewFadeIn = AnimationUtils.loadAnimation(activity, R.anim.progresshud_hudview_fade_in);
        animHudViewFadeOut = AnimationUtils.loadAnimation(activity, R.anim.progresshud_hudview_fade_out);

        infoImage = new ImageView(activity);
        infoImage.setImageResource(R.drawable.progresshud_info);

        successImage = new ImageView(activity);
        successImage.setImageResource(R.drawable.progresshud_success);

        errorImage = new ImageView(activity);
        errorImage.setImageResource(R.drawable.progresshud_error);
        ...
        // Set default values
        setDefaultMaskType(ProgressHUDMaskType.None);
        setDefaultStyle(ProgressHUDStyle.Light);
        setDefaultAnimationType(ProgressHUDAnimationType.Flat);
        setMinimumSize(0, 0);
...
//        _maxSupportedWindowLevel = UIWindowLevelNormal;
    ...
    public void setDefaultStyle(ProgressHUDStyle style) {
        defaultStyle = style;
    ...
    public void setDefaultMaskType(ProgressHUDMaskType maskType) {
        defaultMaskType = maskType;

        backgroundLayer.setClickable(defaultMaskType != ProgressHUDMaskType.None);
    ...
    public void setDefaultAnimationType(ProgressHUDAnimationType type) {
        ...
        if (defaultAnimationType == ProgressHUDAnimationType.Flat && firstChild != indefiniteAnimatedView) {
            imageContainer.removeAllViews();
            imageContainer.addView(indefiniteAnimatedView);
        } else if (defaultAnimationType == ProgressHUDAnimationType.Native && firstChild != activityIndicatorView) {
            imageContainer.removeAllViews();
    ...
    private void show(
        ...
        if (imageView != null) {
            ...
            imageContainer.addView(defaultAnimationType == ProgressHUDAnimationType.Flat ? indefiniteAnimatedView : activityIndicatorView);
        }
    ...
    public void dismissWithCompletion(ProgressHUDDismissCompletion completion) {
    ...
    public void dismissWithDelay(float delay, final ProgressHUDDismissCompletion completion) {
